...
struct destinfo *check(struct in_addr *target)
{
  ...
  for (i = 0; i < num_destinfo; i++) {
    if (debug) fprintf(stderr, "DEBUG: %d: %08x %08x\n",
                      i,
    ...
    if ((destinfos[i].dest.s_addr & bitmask(destinfos[i].size)) ==
        (target->s_addr & bitmask(destinfos[i].size))) {
      if (debug) fprintf(stderr, "DEBUG: match !\n");
      return destinfos + i;
    } else {
      if (debug) fprintf(stderr, "DEBUG: no match \n");
    }
  }
  return NULL;
}
...
void show_destinfos()
{
  int i;
  for (i = 0; i < num_destinfo; i++) {
    if (debug) fprintf(stderr, "DEBUG: %d: %08x %d %08x\n",
                      i,
...
void parse_env()
{
  ...
  if (debug) fprintf(stderr, "DEBUG: MSLICE_TARGETS: %s\n", env);
  ...
  for (i = 0, str1 = env; ; i++, str1 = NULL) {
    ...
    if (debug) fprintf(stderr, "DEBUG: %d: %s\n", i, token1);

    for (j = 0, str2 = token1; ; j++, str2 = NULL) {
      token2 = strtok_r(str2, ",", &saveptr2);
      if (debug) fprintf(stderr, "DEBUG(token2): %d: %s\n", j, token2);
      if (token2 == NULL || j > 1) {
        break;
      }
      if (j == 0) {
        if ((str3 = strchr(token2, (int)'/')) != NULL) {
          netsize = atoi(str3 + 1);
          if (debug) fprintf(stderr, "DEBUG(size): %s (%d)\n", str3 + 1, netsize);
          dest_addr = strndup(token2, str3 - token2);
        } else {
          dest_addr = token2;
          if (debug) fprintf(stderr, "DEBUG(dest): %d %s\n", j, dest_addr);
        }
      }
      if (j == 1) {
        local_addr = token2;
        if (debug) fprintf(stderr, "DEBUG(local):  %d: %s\n", j, local_addr);
      }
    }
    if (debug) fprintf(stderr, "DEBUG2: %s/%d -> %s\n",
                       dest_addr, netsize, local_addr);
    ret = inet_aton(dest_addr, &(destinfos[i].dest));
    if (ret <= 0) {
      if (debug) fprintf(stderr, "DEBUG3: %s is not valid\n", dest_addr);
      continue;
    }
  ...
  num_destinfo = i;
  if (debug) fprintf(stderr, "DEBUG: num_destinfo = %d\n", num_destinfo);
}
...
static void init_hook()
{
  ...
  if (debug) fprintf(stderr, "DEBUG: %s: constructor called! %p\n", __func__, org);
}

__attribute__((destructor))
static void fini_hook()
{
  if (debug) fprintf(stderr, "DEBUG: %s: destructor called!\n", __func__);
}
...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  if(debug) fprintf(stderr, "DEBUG: %s: hook!\n", __func__);
  ...
  if (dinfo != NULL) {
    ...
    if (debug) fprintf(stderr, "DEBUG: bind(2) returns %d (%d)\n", ret, errno);
    if (ret < 0) {
      fprintf(stderr, "WARNING: bind(2) failed\n");
    }
  }
  return org(sockfd, addr, addrlen);
}
